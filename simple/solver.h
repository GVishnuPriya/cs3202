...
class QuerySolver {
  ...
  virtual bool evaluate(SimpleCondition *left_condition, SimpleCondition *right_condition) = 0;
};

/*
 * This is without Memoization Table optimization.
 */
template <typename ConcreteSolver>
class SimpleSolverGenerator : public QuerySolver  {
  public:
    SolverGenerator(std::shared_ptr<SimpleRoot> ast) : _solver(ast) { }
    ...
    virtual bool validate(SimpleCondition *left_condition, SimpleCondition *right_condition) {
        FirstSolverVisitor visitor(&solver, right_condition);
        left_condition->accept_condition_visitor(&visitor);
        return visitor.return_result();
    }
  ...
  private:
    ...
    class SolverLeftVisitor : public ConditionVisitor {
      public:
        ...
        void visit_statement_condition(StatementCondition *condition) {
            _result = _solver->solve_left<StatementAst>(condition->get_statement_ast());
        }

        void visit_proc_condition(ProcCondition *condition) {
            _result = _solver->solve_left<ProcAst>(condition->get_proc_ast());
        }

        void visit_variable_condition(VariableCondition *condition) {
            _result = _solver->solve_left<SimpleVariable>(condition->get_variable());
        }

        void visit_pattern_condition(PatternCondition *condition) {
            _result = _solver->solve_left<ExprAst>(condition->get_expr_ast());
        }
    ...
    class SolverRightVisitor : public ConditionVisitor {
      public:
        ...
        void visit_statement_condition(StatementCondition *condition) {
            _result = _solver->solve_right<StatementAst>(condition->get_statement_ast());
        }

        void visit_proc_condition(ProcCondition *condition) {
            _result = _solver->solve_right<ProcAst>(condition->get_proc_ast());
        }

        void visit_variable_condition(VariableCondition *condition) {
            _result = _solver->solve_right<SimpleVariable>(condition->get_variable());
        }

        void visit_pattern_condition(PatternCondition *condition) {
            _result = _solver->solve_right<ExprAst>(condition->get_expr_ast());
        }
    ...
    class FirstSolverVisitor : public ConditionVisitor {
      public:
        ...
        void visit_statement_condition(StatementCondition *condition) {
            second_visitor = new SecondSolverVisitor<
      ...
      private:
        ...
        std::shared_ptr<ConditionVisitor> second_visitor;
    };
