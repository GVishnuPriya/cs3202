...
TEST(ParentTest, TrivialTest) {
    ...
    EXPECT_FALSE((solver.validate<StatementAst, StatementAst>(loop1, assign1)));
    EXPECT_FALSE((solver.validate<StatementAst, StatementAst>(loop1, loop2)));
    EXPECT_FALSE((solver.validate<StatementAst, StatementAst>(condition1, assign2)));
    EXPECT_FALSE((solver.validate<StatementAst, StatementAst>(condition1, assign4)));

    EXPECT_EQ(solver.solve_left<StatementAst>(condition1), 
                ConditionSet(new SimpleStatementCondition(loop1)));

    EXPECT_EQ(solver.solve_left<StatementAst>(assign1), 
                ConditionSet(new SimpleStatementCondition(condition1)));

    EXPECT_EQ(solver.solve_left<StatementAst>(assign2), 
                ConditionSet(new SimpleStatementCondition(loop2)));

    EXPECT_EQ(solver.solve_left<StatementAst>(assign3), 
                ConditionSet(new SimpleStatementCondition(condition2)));

    EXPECT_EQ(solver.solve_left<StatementAst>(condition2), 
                ConditionSet(new SimpleStatementCondition(condition1)));
