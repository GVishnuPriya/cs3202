...
template<>
bool SiblingSolver::validate<VariableAst, StatementAst>(
    VariableAst *left, StatementAst *right)
{
    if(_sibling_container_index.count(left) == 0) {
        return false;
    } else if(_sibling_container_index[left]
        .count(right) == 0) 
    {
        return false;
    } else{
...
template<>
bool SiblingSolver::validate<StatementAst, VariableAst>(
    StatementAst *left, VariableAst *right)
{
    if(_sibling_container_inverse_index.count(left) == 0) {
        return false;
    } else if(_sibling_container_inverse_index[left]
        .count(right) == 0)
    {
        return false;
    } else {
...
template<>
bool SiblingSolver::validate<ExprAst, ExprAst>(
    ExprAst *left, ExprAst *right)
{
    if(left == right){
        return false;
    }
    ...
    ExprAst *left, OperatorCondition *right)
{
    ...
    if(right_operator == left_expr) {
        return false;
    } else if(_sibling_index[right_operator]
        .count(left_expr) > 0)
    {
        return true;
    } else{
    ...
    ContainerAst *container)
{
    ...
    {
    case IfST:
        return expr_set_to_condition_set(
            _sibling_container_inverse_index[
                statement_cast<IfAst>(container)->get_then_branch()]);
    
    case WhileST:
        return expr_set_to_condition_set(
            _sibling_container_inverse_index[
                statement_cast<WhileAst>(container)->get_body()]);
